...
export function defineOAuthShopifyCustomerEventHandler({
  ...
  return eventHandler(async (event: H3Event) => {
    config = defu(
      config,
      useRuntimeConfig(event).oauth?.shopifyCustomer,
      {}
    ) as OAuthShopifyCustomerConfig;
    ...
    const discoveryResponse: CustomerDiscoveryResponse | null = await $fetch(
    ...
    const tokens: AccessTokenResponse = await requestAccessToken(
      discoveryResponse.token_endpoint,
      {
        body: {
    ...
    // get api
    const apiDiscoveryUrl: CustomerApiDiscoveryResponse | null = await $fetch(
    ...
    const user: ShopifyCustomer | null = await $fetch(
      apiDiscoveryUrl.graphql_api,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          query:
            "query { customer { firstName lastName emailAddress { emailAddress } displayName}}",
        }),
      }
    )
      .then((d) => {
